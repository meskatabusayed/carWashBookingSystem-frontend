...
export default function Component() {
  ...
  const RatingJsx = Rating as any;
  const totalRating = data?.data?.reduce((acc, cur) => acc + cur.rating, 0) || 0;
  const totalCount = data?.data?.length || 1;

  const avgRating = (totalRating / totalCount).toFixed(2);

  return (
    <section
      className="layout_container py-[40px] flex md:flex-row flex-col gap-[20px] relative"
      id="review"
    >
      {!user && (
